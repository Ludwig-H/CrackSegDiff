...
try:
    ...
    try:
        from .selective_scan_pure import selective_scan_fn, selective_scan_ref
    except ImportError:
        pass
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 9, 2], depths_decoder=[2, 9, 2, 2],
        ...
        #     if i_layer != 3 and i_layer != 2:
        #         layer = FFTBlock(dims[i_layer])
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            ...
            x = layer(x)
            # count += 1
            # if count == 1 or count == 3 or count == 4 or count == 6:
            # skip_list.append(x)
        return x, skip_list
...
class SpectralGatingNetwork(nn.Module):
    def __init__(self, dim):
        ...
        if dim == 128: #96 for large model, 64 for small and base model
            self.h = 32 #H
            self.w = 17 #(W/2)+1
            self.complex_weight = nn.Parameter(torch.randn(self.h, self.w, dim, 2, dtype=torch.float32) * 0.02)
    ...
    def forward(self, x):
        ...
        y_fft = torch.fft.rfft2(x, dim=(1, 2), norm='ortho')
        weight = torch.view_as_complex(self.complex_weight)
        y_fft = y_fft * weight
        y_fft = torch.fft.irfft2(y_fft, s=(H, W), dim=(1, 2), norm='ortho')
        return y_fft
