  ...
  "cells": [
    ...
    },
    {
      ...
      "source": [
        ...
        "        \n",
        "        # Row 1: Original\n",
        "        if orig.ndim == 3 and orig.shape[2] >= 2:\n",
        "            axes[0,0].imshow(orig[...,0], cmap='gray'); axes[0,0].set_title('Original Ch0 (Intensity)')\n",
        "            axes[0,1].imshow(orig[...,1], cmap='gray'); axes[0,1].set_title('Original Ch1 (Range)')\n",
        "        else:\n",
        "            axes[0,0].text(0.5, 0.5, f\"Shape {orig.shape} invalid\", ha='center')\n",
        "        axes[0,2].axis('off')\n",
        "        \n",
        "        # Row 2: Reconstructed\n",
        "        axes[1,0].imshow(recon[...,0], cmap='gray'); axes[1,0].set_title('Recon Ch0 (From Int PNG)')\n",
        "        axes[1,1].imshow(recon[...,1], cmap='gray'); axes[1,1].set_title('Recon Ch1 (From Range JET Red)')\n",
        "        \n",
        "        # Diff Maps & Metrics\n",
        "        if orig.shape[:2] == recon.shape[:2] and orig.ndim==3 and orig.shape[2] >= 2:\n",
        "            # Normalize for fair comparison (handle uint8 vs uint16)\n",
        "            def normalize(x):\n",
        "                return (x - x.min()) / (x.max() - x.min() + 1e-8)\n",
        "            \n",
        "            o_rng_norm = normalize(orig[..., 1].astype(float))\n",
        "            r_rng_norm = normalize(recon[..., 1].astype(float))\n",
        "            \n",
        "            # Metrics\n",
        "            diff_norm = np.abs(o_rng_norm - r_rng_norm)\n",
        "            nmae = diff_norm.mean()\n",
        "            # Pearson Correlation\n",
        "            flat_o = o_rng_norm.flatten()\n",
        "            flat_r = r_rng_norm.flatten()\n",
        "            corr = np.corrcoef(flat_o, flat_r)[0, 1]\n",
        "            \n",
        "            # Display\n",
        "            im_diff = axes[0,2].imshow(diff_norm, cmap='hot', vmin=0, vmax=1); \n",
        "            axes[0,2].set_title(f'Norm Diff Range (NMAE={nmae:.3f})')\n",
        "            plt.colorbar(im_diff, ax=axes[0,2])\n",
        "            axes[0,2].axis('on')\n",
        "            \n",
        "            print(f\"--- Quantitative Analysis (Range Channel) ---\")\n",
        "            print(f\"NMAE (0-1 scale): {nmae:.4f} (Lower is better)\")\n",
        "            print(f\"Pearson Correlation: {corr:.4f} (1.0 is perfect match)\")\n",
        "            \n",
        "            if nmae > 0.1 or corr < 0.9:\n",
        "                print(\"⚠️ CRITICAL: Significant structural difference detected.\")\n",
        "                print(\"   -> If NMAE is high and Corr is low, the data content is different (JET artifact?).\")\n",
        "            else:\n",
        "                print(\"✅ OK: Structure matches well.\")\n",
        "        else:\n",
        "            print(\"⚠️ Dimension mismatch or invalid shape, cannot compute pixel diff.\")\n",
        "        \n",
        "        plt.tight_layout()\n",
        "        plt.show()\n",
        "    except Exception as e:\n",
