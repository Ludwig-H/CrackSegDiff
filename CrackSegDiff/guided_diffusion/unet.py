...
class UNetModel_newpreview(nn.Module):
    ...
    def __init__(
        ...
        # self.s1 = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)
        # self.s2 = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)
        if num_heads_upsample == -1:
            num_heads_upsample = num_heads
        ...
        # self.ffp4 = FFParser(dim=256, h=32, w=17)
        # self.ffp5 = FFParser(dim=256, h=64, w=33)
        # self.CoAtt = CSCA(512)
    def convert_to_fp16(self):
        """
        Convert the torso of the model to float16.
    ...
    def forward(self, x, timesteps, y=None):
        ...
        for ind, module in enumerate(self.output_blocks):
            hs_ = hs.pop()
            # if hs_.shape[1] == 512 and hs_.shape[2] == 8:
            ...
            #     # h[:, :] = h[:, :] * ((self.b1 - 1) * hidden_mean + 1)
            #     hs_ = self.ffp1(hs_)
            # if hs_.shape[1] == 512 and hs_.shape[2] == 16:
            #     # hidden_mean = h.mean(1).unsqueeze(1)
            #     # B = hidden_mean.shape[0]
            #     # hidden_max, _ = torch.max(hidden_mean.view(B, -1), dim=-1, keepdim=True)
            #     # hidden_min, _ = torch.min(hidden_mean.view(B, -1), dim=-1, keepdim=True)
            ...
            #     # h[:, :] = h[:, :] * ((self.b2 - 1) * hidden_mean + 1)
            #     if ind == 6:
            #         hs_ = self.ffp3(hs_)
            #     if ind == 7 or ind == 8 or ind == 9:
            #         hs_ = self.ffp4(hs_)
            #     if ind == 10 or ind == 11:
            #         hs_ = self.ffp5(hs_)
            # if ind == 0:
            ...
            h_.append(h)
            if ind == 2:
                hs_ = self.att1(hs_, h_[ind-1], hs[6])
            if ind == 5:
                hs_ = self.att2(hs_, h_[ind-1], hs[6])
...
class SqueezeAndExcitation(nn.Module):
    ...
    def forward(self, x):
        ...
        weighting = self.sigmoid(weighting)
        return weighting
...
class CSCA(nn.Module):
    def __init__(self, channels_in):
        ...
        self.se = SqueezeAndExcitation(channels_in)
        self.channel = channels_in
        self.hidden_size = channels_in * 2
        self.sa = CrossBlock(mode='sa', hidden_size=self.hidden_size)
        self.mba = CrossBlock(mode='mba', hidden_size=self.hidden_size)
        self.PatchEmbed2D = PatchEmbed2D(in_chans=channels_in, embed_dim=self.hidden_size)
        self.ConvTranspose2dRgb = nn.ConvTranspose2d(self.hidden_size, self.hidden_size, kernel_size=4, stride=4)
        self.ConvTranspose2dDepth = nn.ConvTranspose2d(self.hidden_size, self.hidden_size, kernel_size=4, stride=4)
    def forward(self, rgb, depth):
        input = rgb + depth
        weighting1, weighting2 = self.se(input)
        rgb = rgb * weighting1
        depth = depth * weighting2
        # rgb.reshape(B, C, -1).permute(0, 2, 1)
        # depth.reshape(B, C, -1).permute(0, 2, 1)
        rgb, depth = self.PatchEmbed2D(rgb, depth)
        B, H, W, C = rgb.size()
        rgb = rgb.reshape(rgb.shape[0], -1, rgb.shape[3])
        depth = depth.reshape(depth.shape[0], -1, depth.shape[3])
        rgb, depth = self.sa(rgb, depth)
        rgb, depth = self.mba(rgb, depth)
        rgb = self.ConvTranspose2dRgb(rgb.reshape(B, C, H, W))
        depth = self.ConvTranspose2dDepth(depth.reshape(B, C, H, W))
        out = rgb + depth
        return out
...
class SqueezeAndExciteFusionAdd(nn.Module):
    def __init__(self, channels_in):
        ...
        self.rgb = SqueezeAndExcitation(channels_in)
        self.depth = SqueezeAndExcitation(channels_in)
        self.channel = channels_in
        self.spatial = SpatialAttention()
    def forward(self, rgb, depth):
        input = rgb + depth
        weighting1 = self.rgb(input)
        weighting2 = self.depth(input)
        rgb = rgb * weighting1
        depth = depth * weighting2
        out = self.spatial(rgb + depth)
        return out

class CrossAttention(nn.Module):
    def __init__(self, vis, mode=None, hidden_size=None):
        ...
        self.queryd = nn.Linear(self.hidden_size, self.all_head_size)
        self.keyd = nn.Linear(self.hidden_size, self.all_head_size)
        self.valued = nn.Linear(self.hidden_size, self.all_head_size)
        self.outd = nn.Linear(self.hidden_size, self.hidden_size)
    ...
    def forward(self, hidden_statesx, hidden_statesy):
        ...
        if self.mode == 'mba':
            ...
            return attention_cx, attention_cy
...
class CrossBlock(nn.Module):
    def __init__(self, mode=None, hidden_size=None):
        super(CrossBlock, self).__init__()
        self.hidden_size = hidden_size
        self.attention_norm = nn.LayerNorm(self.hidden_size, eps=1e-6)
        self.attention_normd = nn.LayerNorm(self.hidden_size, eps=1e-6)
        self.ffn_norm = nn.LayerNorm(self.hidden_size, eps=1e-6)
        self.ffn_normd = nn.LayerNorm(self.hidden_size, eps=1e-6)
        self.ffn = Mlp(hidden_size=self.hidden_size)
        self.ffnd = Mlp(hidden_size=self.hidden_size)
        self.attn = CrossAttention(vis=False, mode=mode, hidden_size=self.hidden_size)
...
class PatchEmbed2D(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None, **kwargs):
        ...
        self.projx = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)
        self.projy = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)
    ...
    def forward(self, x, y):
        x = self.projx(x).permute(0, 2, 3, 1)
        y = self.projy(y).permute(0, 2, 3, 1)
...
class CSA(nn.Module):
    def __init__(self, channel_l, channel_g, init_channel=64, mode=None):
        super(CSA, self).__init__()
        self.W_Fp = nn.Conv2d(channel_l, channel_l, kernel_size=1)
        self.stride = mode
        self.W_Fp_ = nn.Conv2d(channel_l, channel_g, kernel_size=3,
                              stride=1, padding=1)
        self.W_Ff = nn.Conv2d(init_channel, channel_l, kernel_size=3,
                              stride=self.stride, padding=1)
        self.W_Fq = nn.Conv2d(channel_g, channel_g, kernel_size=1)
        self.relu = nn.ReLU()
        self.f1 = nn.Conv2d(channel_l, out_channels=1, kernel_size=1)
        self.f2 = nn.Conv2d(channel_g, out_channels=1, kernel_size=1)
        self.sig = nn.Sigmoid()

    def forward(self, x_l, x_g, first_layer_f):
        Ff = self.W_Ff(first_layer_f)
        Fp = self.W_Fp(x_l)
        W_Ffp = self.sig(self.f1(self.relu(Ff + Fp)))
        Ffp = x_l * W_Ffp
        Fq = self.W_Fq(x_g)
        Fp_ = self.W_Fp_(Ffp)
        W_Fqp_ = self.sig(self.f2(self.relu(Fq + Fp_)))
        up = F.interpolate(W_Fqp_, size=x_l.size()[2:], mode='bilinear')
        out = Ffp * up
        return out
