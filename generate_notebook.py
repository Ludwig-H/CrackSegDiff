  ...
  "cells": [
    ...
    },
    {
      ...
      "source": [
        ...
        "\n",
        "id_img = 'im00002'\n",
        "# 1. Original Fused TIF (from training data)\n",
        "path_orig = f'data/data/img/fused/{id_img}.tif'\n",
        ...
        "    print(f\"ERROR: Original file not found at {path_orig}\")\n",
        ...
        "    print(f\"Noisy Int Shape: {n_int.shape} (Gray?), Noisy Range Shape: {n_rng.shape} (JET?)\")\n",
        "\n",
        ...
        "        diff_rng = np.abs(orig[...,1].astype(int) - recon[...,1].astype(int))\n",
        "        im_diff = axes[0,2].imshow(diff_rng, cmap='hot'); axes[0,2].set_title(f'Diff Range (MAE={diff_rng.mean():.2f})'); plt.colorbar(im_diff, ax=axes[0,2])\n",
        "        axes[0,2].axis('on')\n",
        "        print(f\"Range Channel MAE: {diff_rng.mean():.4f}\")\n",
        "        if diff_rng.mean() > 50:\n",
        "            print(\"⚠️ CRITICAL: Huge difference in Range channel.\")\n",
        "        else:\n",
        "            print(\"✅ OK: Range channel matches.\")\n",
        "    else:\n",
        "        print(\"⚠️ Dimension mismatch, cannot compute pixel diff.\")\n",
        "    \n",
        "    plt.tight_layout()\n",
        "    plt.show()"
      ],
