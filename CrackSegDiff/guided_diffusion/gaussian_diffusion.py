...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.BceDiceLoss_diff = BceDiceLoss()
        self.BceDiceLoss_unet = BceDiceLoss()
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.BCE_DICE or self.loss_type == LossType.RESCALED_MSE:
            ...
            # terms["loss_diff"] = self.bceloss(model_output, target)
            terms["loss_diff"] = mean_flat((target - model_output) ** 2 )
            terms["loss_cal"] = self.BceDiceLoss_diff(cal.type(th.float), res.type(th.float))
            # terms["loss_unet"] = self.BceDiceLoss_unet(model_var_values.type(th.float), res.type(th.float))
            # terms["loss_cal"] = self.DiceBCELoss(cal.type(th.float), res.type(th.float))
            # terms["loss_cal"] = mean_flat((res - cal) ** 2)
            # terms["loss_cal"] = self.bceloss(cal.type(th.float), res.type(th.float))
